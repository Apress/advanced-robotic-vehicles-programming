...
char readd;
uint32_t begin=0,ending=0, timer, timemod;
...
void loop(void)
{
// a function scheme is used as required for real time
    fast_loop();
}
...
// the current time is invoked
    timer=hal.scheduler->millis();

// you get the module between the time you want to display the data on the screen
// here every 3 seconds or equivalently 3000 milliseconds

    timemod=timer%3000;

// if the module is exactly 3 seconds or around (2.9 seconds)

    if(timemod<=100)
    {

// displaying the value of the chronometer as at the beginning
// there are not start and end values these were initialized to 0 in the declaration

        hal.console->printf("%d\n",ending-begin);
    }


// the fast loop must contain at least the ahrs.update

    ahrs.update();

// here starts the chronometer, we are interested in measuring the task a and b that we // know have about 56,000 microseconds long
 
    begin=micros();
    atask();
    btask();
// here the chronometer stops
    ending=micros();
}
...
// tasks a and b last together approximately 56 milliseconds 11 + 45
// those are approximately 56,000 microseconds
// they are used like this to show the reader their usefulness, but in general it is 
// unknown the duration of an specific task
  
static void atask(void){
...
// this function is defined to simplify writing
