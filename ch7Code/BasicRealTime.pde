...
static const AP_Scheduler::Task scheduler_task[] PROGMEM = {
        {onesec,       400,  10}, //400 dvided by 400 is 1 s
        {threesec,       1200,  10}, // 1200 divided by 400 are 3 s
        {fivesec,     2000,  10},  // 2000 divided by 400 are 5 s
};
...
void loop(void)
{
    ...
    ins.wait_for_sample();// the inertial sensor determines the use of the scheduler
    uint32_t timer =  micros();
    fast_loop();  // fast loop is ejecuted 
    scheduler.tick(); // fast loop ends
    uint32_t time_available=(timer+MAIN_LOOP_MICROS)-micros(); // current time
    scheduler.run(time_available); // execute what is defined in the scheduler
                                    // if there is time available

}


// the fast loop is not declared in the scheduler and the tasks within should be
// only the most relevant, in this case only ahrs is update but you can add the control // loop and writing to the engines
...
static void onesec(){
    hal.console->printf("one \n");
}

static void threesec(){
    hal.console->printf("three \n");
}

static void fivesec(){
    hal.console->printf("five \n");
}
