...
// modulation to write every 3.5 milliseconds if you have servomotors,
// for BLDC motors this operation can be omitted and simply send directly
// the radio input to the motor
...
// using throttle channel to test an engine
    radio_throttle = radio[2];

// write the radio signal to the chosen motor according to the selected time base
    if(timemod==0)
    {
    hal.rcout->write(0,radio_throttle);

// it is possible that in some pixhawk models the value to be written to the motor must 
// be converted in this way hal.rcout-> write (0, uint16_t (radio_throttle));
    }    
