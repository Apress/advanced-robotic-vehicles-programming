...
uint8_t _buffertx[4];
uint8_t _bufferrx[4];
int permit=0; // the other device starts as a reader
...
void loop(void)
{
  ...
  {
    writeSerial();
    permit=1; // once device2 has written, it begins to read
  }
...
static void writeSerial(){
    ...
    int envi1=1099;
    _buffertx[0]=1; // sending its turn to device 1
    _buffertx[1]=envi1>>8;       
    _buffertx[2]=envi1 & 0x00FF;
...
static void leeSerial(){
    ...
    permit=_bufferrx[0];  // in this line the permission to write is retaken
