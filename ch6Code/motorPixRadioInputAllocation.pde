...
///// reading of all the radio channels, radio must be in mode 2 or helicopter mode
...
///// assignment of each channel 
    radio_roll  = (radio[0]-1500)/3;
    radio_pitch = (radio[1]-1510)/3;
    radio_throttle = radio[2];
    radio_yaw = (radio[3]-1510)/2;
    aux_1 = radio[4];
    aux_2 = radio[5];

///// values to write from
///// remote control to each engine through its corresponding
///// allocation matrix type X (see further sections), notice the saturation to the engines
///// so that they do not exceed the minimum and maximun security values


    m1 = satu((radio_throttle-radio_roll+radio_pitch+radio_yaw),1700,1100);
    m2 = satu((radio_throttle+radio_roll-radio_pitch+radio_yaw),1700,1100);
    m3 = satu((radio_throttle+radio_roll+radio_pitch-radio_yaw),1700,1100);
    m4 = satu((radio_throttle-radio_roll-radio_pitch-radio_yaw),1700,1100);

// emergency stop using a two-position auxiliary lever,
// when the lever is on, it is written to each motor
// otherwise, the motors are switched off

    if (aux_1<1500)
    {
        hal.rcout->write(0,uint16_t(m1));
        hal.rcout->write(1,uint16_t(m2));
        hal.rcout->write(2,uint16_t(m3));
        hal.rcout->write(3,uint16_t(m4));
    }
...
static float satu(float nu, float maxi, float mini){
    if(nu>=maxi) nu=maxi;
    else nu=nu;
    if(nu <= mini) nu=mini;
    else nu=nu;
