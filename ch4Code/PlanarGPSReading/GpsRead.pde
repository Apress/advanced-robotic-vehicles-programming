...
// a delta t is required to internally calculate velocities
      inertial_nav.update(dt);
     
// this part verifies that there are at least 3 satellites to operate and turn on the led if this is affirmative, also changes a variable called flag2 to update speeds
      ...
      if(pos.x!=0 && flag >=3 && flag2==1){
           ...
           toshiba_led.set_rgb(0,LED_DIM,0);   // green LED
           flag2 = 2;
        }
...
// and then it is transformed to meters
...
// utility to display the GPS data, comment if it is not needed
//hal.console->printf("%f\t %f\t %f\t %f\t //%d\n",pos_gps.x,pos_gps.y,vel_gps.x,vel_gps.y,gps.num_sats());
